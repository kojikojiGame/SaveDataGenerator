...
using UnityEngine;
using static STDB_PlayerMoneyRecordMaster;
using System.Linq;
...
// コピペ用
// PlayerMoneyRecordMaster

[CreateAssetMenu]
public class STDB_PlayerMoneyRecordMaster : ScriptableObject, IMasterData
{
    public static string resourcesPath = "DT_PlayerMoneyRecordMaster";

    [System.Serializable]
    public class ST_PlayerMoneyRecordMaster
    {
        public int Id;
        public string TextKeyName;
    }

    public ST_PlayerMoneyRecordMaster[] datas;
...
#if UNITY_EDITOR
public class EditorMenuCsv2DT_PlayerMoneyRecordMaster : MonoBehaviour
{
    //　項目を作成
    [MenuItem("CsvImport/PlayerMoneyRecordMaster", false, 1)]
    static void CsvImportPlayerMoneyRecordMaster()
    {
        //--------------------------------------------------
        // ＝＝＝GameText＝＝＝
        string str = "Assets/Master/Resources/CSV_PlayerMoneyRecordMaster.csv";
        ...
        STDB_PlayerMoneyRecordMaster md = AssetDatabase.LoadAssetAtPath<STDB_PlayerMoneyRecordMaster>(assetfile);
        if (md == null)
        {
            md = new STDB_PlayerMoneyRecordMaster();
            AssetDatabase.CreateAsset(md, assetfile);
        }

        //　※"datas"もScriptableObjectが保有する配列名に合わせる。
        md.datas = CSVSerializer.Deserialize<STDB_PlayerMoneyRecordMaster.ST_PlayerMoneyRecordMaster>(textasset.text);
